...
def playGame():
    ...
    while True:
        ...
        if(nowTime < blockTime):
            if ((blockTime - nowTime) // 1) % 200 < 5:
                print("Waiting", blockTime - nowTime)
            time.sleep(5)
            continue
        ...
        if abs(pos[0] - playPostion[0]) < 20 and abs(pos[1] - playPostion[1]) < 20:
            ...
            time.sleep(2)
            screen = takeScreenShot()
            if len(findImage(pauseImg, screen)) > 0 or len(findImage(banthangImg, screen)) > 0:
                click(playPostion[0], playPostion[1])
                count = count + 1
                print("Thuc hien lan thu", count)
                tmClickTime = time.time()
        ...
        pastTime = time.time() - tmClickTime
        if checkDongHo and pastTime > 27:
            checkDongHo = False
            screen = takeScreenShot()
        ...
        if lamMoiXang and pastTime > 10 and pastTime < 20:
            screen = takeScreenShot()
            xangImg = screen[xangRect[1]:xangRect[3], xangRect[0]:xangRect[2]]
